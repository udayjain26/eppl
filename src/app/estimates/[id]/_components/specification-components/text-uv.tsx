...
export default function TextUV(props: {
  control: any
  form: UseFormReturn
  product: string
}) {
                  ...
                  <CommandList>
                    {uvTypes.map((size) => (
                      <CommandItem
                        ...
                        value={size.label}
                        onSelect={() => {
                          props.form.setValue('textUV', size.label)
                          field.onChange(size.label)
                          props.form.trigger('textUV') // Trigger form validation and state update
